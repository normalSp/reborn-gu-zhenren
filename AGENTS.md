...
- `v0.18.0` startup review and scope-freeze draft exists as `南疆路线与多区域承接`. Proposed sequence is `a1` route/location/save/BFF design gate, `a2` MiroFish intake + field table + matrix, `b1` Qingmao route-entry threshold sample, `b2` route state first cut or candidate-continuation fallback, `b3` Southern Border low-rank public fact cards and route panel, `b4` previous-ledger supply/pursuit/identity pressure, `b5` caravan/rogue/Shang-clan-outer public-entry boundary, then `rc`. MiroFish requests are drafted for `v018_route_entry_state_and_milestones_pack`, `v018_southern_border_low_rank_region_fact_cards_pack`, and `v018_post_qingmao_pressure_reaction_pack`; the user must forward them to the MiroFish thread before blocking/preferred phases. Do not enter runtime work, add route/location/save fields, bump save format, unlock regions/factions/rewards/NPC-life outcomes, open full Southern Border or full Shang clan city, add backend/BFF, or expand DeepSeek authority until the user approves the v0.18 decisions.
