...
- `v1.0.0` startup review and scope-freeze draft is established under `指导大纲/v1.0.0/codex/00-总览/`. Proposed mainline is `活世界早期正式版`: Qingmao to early Southern Border continuity, low-rank Gu Master life loop, free-intent adjudication closure, NPC/faction/route backflow, public safety, and long-playtest closure. Required user decisions live in `v1.0.0-需求决策池.md`; MiroFish v1.0 request drafts live under `指导大纲/vMiroFish/requests/v1.0.0/`. Startup package commit `515d0c2` is pushed and GitHub Actions run `26088300312` passed. Do not start b1 runtime absorption until required user decisions and relevant MiroFish intake review are complete. Stop before `SAVE_FORMAT_VERSION = 23`, route/location persistent fields, formal route/location/faction/reward/NPC-life outcomes, BFF/backend implementation, auto deployment, public commitments, large new image generation, MiroFish runtime authority, or DeepSeek authority expansion.
