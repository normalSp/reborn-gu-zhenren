...
- `v0.9.0` through `v1.4.0` are completed or locked milestones. Their phase lists, commit/run evidence, and detailed records are indexed in `指导大纲/historical-index.md`; do not treat old version docs as current gates unless AGENTS, PROJECT-STATE, current version docs, or a project-level process explicitly points to them.
...
- `v1.5.0-a1` is the current active draft: `冲突、追杀、杀招与小队后果深化` save-format / authority design gate. User approved D-150-001 through D-150-010. D-151-001 through D-151-010 are pending user approval; do not enter a2 or runtime before approval. Expert-council recommendation after a1: keep b1 at `SAVE_FORMAT_VERSION = 24`, add no `conflictConsequenceState` / `pursuitState` / `combatAftermathState`, add only a pure `V150ConflictAftermathProjection` or equivalent read-only helper, limit conflict posture to route ambush risk / pursuit attention window / countermeasure gap / squad-formation readiness, and run focused unit + old-save + hidden/death/reward/DeepSeek authority + 30-round Player Advocate + T0-lite. Default hard stops remain: no formal drop pool, rare Gu, Immortal Gu, complete killer-move inheritance, NPC death/capture/betrayal/permanent injury, formal warrant/pursuit/faction hostility conclusion, DeepSeek combat/reward/death authority, EdgeOne deployment, or public wording. MiroFish starts as `preferred`; it becomes `blocking` for named NPC/original-character conflict outcomes, killer-move canon inheritance, hidden-adjacent material, NPC life/death, canon promotion, or runtime/DeepSeek/player-visible fact use. Next step after user approval is `v1.5.0-a2-MiroFish-战斗追杀杀招小队topic-slice-intake.md`.
...
For fuller current facts, read `.codex/skills/reborn-expert-council/references/PROJECT-STATE.md` first, then `指导大纲/v1.5.0/codex/00-总览/README.md`; use `指导大纲/historical-index.md` before opening old version phase lists.
...
- Current active draft docs: `指导大纲/v1.5.0/codex/00-总览/`.
- Latest completed milestone docs: `指导大纲/v1.4.0/codex/00-总览/`.
- Previous completed milestone docs: `指导大纲/v1.3.0/codex/00-总览/`.
- Older completed milestone docs: `指导大纲/v1.2.0/codex/00-总览/`.
