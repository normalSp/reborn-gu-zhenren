...
- `v1.3.0-b1` is the current active draft: `NPC 与势力长期关系第二层`. User approved D-130-001 through D-130-009 and D-131-001 through D-131-007. b1 has completed the social-pressure projection-only first cut: world panel adds `社会`, `buildV130SocialPressureProjection()` reads existing `livingWorldState.knownFacts`/`npcMemories`/`factionPressure`/`actionConsequences` and local action ledger, and the UI shows faction pressure, memory traces, public events, and social follow-up candidates. b1 stays projection-only, does not bump `SAVE_FORMAT_VERSION = 25`, does not add `socialRelationState`, and keeps legacy fields like `npcRelations`/`standings`/`npcContacts`/`dynamicNPCs` out of v1.3 authority unless a later normalization gate promotes them. a2 reuses v0.13 MiroFish NPC memory/motive, faction pressure, and public event chronicle packs as candidate/rule/test material; the full-book base is only archive/source-pointer inventory and coverage reference. Formal named NPC allowlists, wanted/recruitment/blockade conclusions, hidden-adjacent/Fang Yuan public-evidence consequences, NPC life/death, or canon promotion remain blocking and require separate user approval. v1.3-rc live probe is approved in principle with model fixed to `deepseek-v4-flash`, but cost, sample count, rounds, and acceptance criteria still require user confirmation at rc. Current docs: `指导大纲/v1.3.0/codex/00-总览/`, especially `v1.3.0-b1-关系证据与社会压力projection-only第一刀.md`, `v1.3.0-b1-Player-Advocate-30轮走查记录.md`, and `v1.3.0-b1-长线叙事漂移检查记录.md`, plus project process `指导大纲/流程制度/Skill同步审计制度.md`.
