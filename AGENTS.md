...
- `v1.0.0` is underway as `活世界早期正式版`. User approved D-001 through D-010 on 2026-05-19. MiroFish v1.0 three packs passed intake review and remain candidate-only. `v1.0.0-a0/a1/a2` are complete. `v1.0.0-b1` 青茅到南疆早期连续体验第一刀 is complete: `src/canon/v100-qingmao-southern-border-continuity-rules.json`, `src/engine/v100-qingmao-southern-border-continuity.ts`, focused tests, store/UI bridge in `FreeGoalPanel`, and `tests/e2e/v100-qingmao-southern-border-continuity.spec.ts` turn v0.18 route preparation, candidate continuation, and pressure backflow into a v1.0 continuity acceptance action. It writes only existing v22 `livingWorldState` fields plus local ledger/return context, and adds no `route_entered/currentRoute/currentRegion`, no save-format bump, no formal location/faction/reward/NPC-life result, no BFF/backend, no public commitment, no MiroFish runtime authority, and no DeepSeek authority expansion. Local b1 gate passed focused unit, `tsc`, b1 e2e, 30-round Player Advocate gate, full unit tests, build, runtime/Qingmao/player-visible-copy scans, v0.19 content governance scan, and production-preview smoke. Commit `595bdc0 feat: 验收v1.0青茅南疆连续体验` is pushed, and GitHub Actions run `26091674479` passed deterministic quality gate. Next step is `v1.0.0-b2` low-rank Gu Master life-loop release closure. Stop before `SAVE_FORMAT_VERSION = 23`, route/location persistent fields, formal route/location/faction/reward/NPC-life outcomes, BFF/backend implementation, auto deployment, public commitments, large new image generation, MiroFish runtime authority, or DeepSeek authority expansion.
