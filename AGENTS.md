...
- `v0.15.0` is active as `低阶蛊师经济、补给、炼养用深循环`. `v0.15.0-a1` design gate and `v0.15.0-a2` candidate rule-pool first cut are complete: 24 current low-rank economy rule drafts plus 10 deferred gray-trade boundaries are read-only previews and add no save fields, rewards, materials, Gu, recipes, market/black-market/commission systems, or DeepSeek authority. `v0.15-art-audit-1` is complete: user-approved battle candidates and v1.0 hero candidates are archived/registered, `qingmao-visual-assets.json` has 23 entries with active=4/candidate=12/review-only=6/blocked=1, and asset scans pass. `v0.15.0-b1` is complete and pushed as `8ec823c feat: 接入v0.15补给喂养行动样板`; GitHub Actions run `26019950577` passed. `v0.15.0-b2` is complete and pushed as `03a2f88 feat: 接入v0.15炼蛊残方边界样板`; GitHub Actions run `26022209682` passed. `v0.15.0-b3` is complete and pushed as `a3dedde feat: 接入v0.15商队市场窗口样板`; GitHub Actions run `26025266822` passed. `v0.15.0-b4` is complete and pushed as `9872c28 feat: 接入v0.15灰色交易边界样板`; GitHub Actions run `26027822854` passed. `qingmao_gray_trade_boundary_probe` turns the accepted gray-trade/commission boundary package into deferred risk state and FreeGoalPanel feedback through existing v22 fields only. It writes no `materialBag`, inventory, currency, price table, shop inventory, item purchase/sale, caravan join, market/black-market/commission settlement, commission profit, identity wash, formal faction pressure, route entry, location/faction/NPC-life result, save field, reward, or DeepSeek authority. Next step is `v0.15.0-rc` quality closure. Stop before formal materials/inventory consumption, recipe unlock/refinement success, market/black-market/commission systems, save-format changes, location/faction/NPC-life results, or DeepSeek authority expansion without user decision.
