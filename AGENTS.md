...
- `v0.14.0-b4` is complete: `FreeGoalPanel` now has a read-only `优先摘要` section summarizing current goal, route continuation, faction/identity prerequisites, and social impact before the detailed panels. It does not write store state, add save fields, change location/faction, create formal tasks, grant rewards, reveal hidden facts, or expand DeepSeek authority. `npx tsc --noEmit --pretty false`, `npm run test:e2e -- tests/e2e/v014-free-goal-summary-priority.spec.ts`, the 20-round Player Advocate gate, full unit tests, build, player-visible-copy, runtime/Qingmao asset scans, production-preview smoke, and GitHub Actions run `25992906897` pass. Implementation commit is `d5ee422`; next recommended step is `v0.14.0-rc 质量收束`. Stop before formal route state, location change, save-format bump, complete Southern Border, full Shang clan city, faction transfer, formal standing/warrant/recruitment/tasks/rewards, NPC death/capture, BFF/backend, EdgeOne auto-deploy, or DeepSeek authority expansion without user decision.
...
For fuller current facts, read `.codex/skills/reborn-expert-council/references/PROJECT-STATE.md` first, then `指导大纲/v0.14.0/codex/00-总览/README.md`, then `指导大纲/v0.13.0/codex/00-总览/README.md`.
...
- Current formal draft docs: `指导大纲/v0.14.0/codex/00-总览/`
- Previous completed draft docs: `指导大纲/v0.13.0/codex/00-总览/`, `指导大纲/v0.12.0/codex/00-总览/`
