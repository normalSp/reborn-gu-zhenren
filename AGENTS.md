...
- `v0.14.0` is approved for planning as `青茅后续路线承接`. The first documentation/rules package exists under `指导大纲/v0.14.0/codex/00-总览/`: overview, scope freeze, roadmap, demand pool, truth-source index, MiroFish protocol, route-continuation rules, Player Advocate round-upgrade rule, Git plan, and a1 design/field/test gates. Next recommended step is `v0.14.0-a1 路线承接设计门禁`, then route-condition runtime work only after the gate is satisfied. Do not open complete Southern Border, full Shang clan city, faction transfer, formal standing/warrant/recruitment/tasks/rewards, location unlocks, NPC death/capture, save-format bump, BFF/backend, EdgeOne auto-deploy, or DeepSeek authority expansion without user decision.
