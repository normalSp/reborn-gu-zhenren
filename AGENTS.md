...
- `v0.19.0` is complete as a local development milestone: `内容生产、长测与 v1.0 发布工具`. User approved D-001 through D-008 on 2026-05-19. MiroFish v0.19 public canon boundary, representative playthrough anchor, and release art caption boundary packs passed intake review and were absorbed only as RebornG-owned candidate/rule/test/caption boundary material. Runtime/tooling adds `src/canon/v019-content-governance-rules.json`, `src/engine/v019-content-governance.ts`, focused tests, and `scripts/check-v019-content-governance.mjs`; it adds no save fields and gives no new DeepSeek authority. `v0.19.0-rc` passed `tsc`, full unit tests, build, runtime/Qingmao/player-visible-copy scans, v0.19 content governance check, full e2e, long e2e, production-preview smoke, b2 30-round Player Advocate gate, and rc 100-round Player Advocate gate. Commit `fe6f468 feat: 完成v0.19发布准备工具层` is pushed, and GitHub Actions run `26082464543` passed deterministic quality gate. Stop before route/location persistent fields, `SAVE_FORMAT_VERSION = 23`, formal route/location entry, region/faction/reward/NPC-life outcomes, complete Southern Border, full Shang clan city, BFF/backend, automatic deployment, public commitments, large new image generation, MiroFish runtime authority, or DeepSeek authority expansion.
- `v1.0.0` is complete as a local development milestone: `活世界早期正式版`. User approved D-001 through D-010 on 2026-05-19. MiroFish v1.0 three packs passed intake review and remain candidate-only. `v1.0.0-a0/a1/a2`, `b1` Qingmao-to-Southern-Border continuity, `b2` low-rank life loop, `b3` free/extreme intent release closure, `b4` public copy/art boundary, `process-1` preview/rollback/observability checklist, and `rc` quality closure are complete. Key commits and runs: b1 `595bdc0` / `26091674479`, b1 evidence `b0d4221` / `26091861120`, b2 `c21ce9a` / `26093300607`, b3 `352948e` / `26095586533`, b3 evidence `611a1ef` / `26096235442`, b4 `54ed13d` / `26096913115`, b4 evidence `d09a44b` / `26097260987`, process-1 `0c2ed24` / `26097693808`, process evidence `43d7578` / `26097887191`, rc `53bdf98` / `26098733549`. `v1.0.0-rc` passed `tsc`, full unit tests, build, runtime/Qingmao/player-visible-copy scans, v0.19 content governance scan, full e2e, long e2e, production-preview smoke, and 150-round Player Advocate gate. It adds no save fields, no route/location persistent fields, no formal route/location/faction/reward/NPC-life outcomes, no formal economic inventory/market system, no BFF/backend, no auto deployment, no public commitment, no MiroFish runtime authority, and no DeepSeek authority expansion. Stop before `SAVE_FORMAT_VERSION = 23`, route/location persistent fields, formal route/location/faction/reward/NPC-life outcomes, formal economic inventory/market systems, BFF/backend implementation, auto deployment, public commitments, large new image generation, MiroFish runtime authority, or DeepSeek authority expansion.
- `v1.0.0` public release prep entered finalization on 2026-05-20 after user approved D-PUB-001 through D-PUB-007. Version identifiers are `v1.0.0`; announcement title is `RebornG v1.0《青茅之后，活世界初成》`; announcement body, hero three-pack, screenshot/short-video directions, and “正式发布” wording are approved. Title screen hero and OG share image are bound as release presentation assets, but they are not gameplay/canon authority. Final local release verification passed: focused v019 unit, content-governance scan, v100 public-release e2e, `tsc`, runtime/Qingmao/player-visible-copy scans, full unit tests, build, full e2e, long e2e, and production-preview smoke. EdgeOne deployment remains user-owned via CodeBuddy; Codex must not auto-deploy.
- `v1.1.0` is the current planning draft: `路线、地点与区域状态地基`. User approved D-001 through D-010, including option A, a0/a1/a2 before runtime, Qingmao -> Southern Border early outer edge only, no BFF/backend, no DeepSeek authority expansion, and 180-round rc Player Advocate if save format is bumped. Pre-v1.1 bug cleanup is recorded in `指导大纲/v1.1.0/codex/00-总览/v1.1.0-preflight-bug清理记录.md` and `codex上下文信息/2026-05-19-v110-preflight-bugfix-context.md`. The design gate may seriously evaluate `SAVE_FORMAT_VERSION = 23`; actual save-format bump, route/location/currentRegion persistent fields, formal location/faction/reward/NPC-life outcomes, DeepSeek authority expansion, BFF/backend, automatic deployment, and public commitments still require explicit user decision. Current docs: `指导大纲/长期路线/RebornG-v1.1至v2.0长期路线重整草案.md`, `指导大纲/长期路线/RebornG-外部活世界参考映射.md`, `指导大纲/v1.1.0/codex/00-总览/`, and v1.1 MiroFish intake review `指导大纲/vMiroFish/intake-reviews/v1.1.0/2026-05-19-v110-three-pack-intake-review-summary.md`.
...
For fuller current facts, read `.codex/skills/reborn-expert-council/references/PROJECT-STATE.md` first, then `指导大纲/v1.1.0/codex/00-总览/README.md`, then `指导大纲/v1.0.0/codex/00-总览/README.md`.
...
- Current active draft docs: `指导大纲/v1.1.0/codex/00-总览/`.
...
- v0.19 completion docs: `指导大纲/v0.19.0/codex/00-总览/README.md`, `v0.19.0-总体开发大纲.md`, `v0.19.0-启动审查与范围冻结.md`, `v0.19.0-小版本执行路线图.md`, `v0.19.0-MiroFish资料需求与交付协议.md`, `v0.19.0-测试矩阵.md`, `v0.19.0-真相源索引.md`, `v0.19.0-需求决策池.md`, `v0.19.0-Git提交与推送计划.md`, `v0.19.0-rc-质量收束记录.md`, and `v0.19.0-rc-Player-Advocate-100轮走查记录.md`
