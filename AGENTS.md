...
- `v1.2.0` is the current active draft: `低阶蛊师生存与经济正式化第一阶段`. User approved D-120-001 through D-120-010, D-121-001 through D-121-007, and D-122-001 through D-122-007. b1 projection-only runtime is complete. b2 local implementation/verification is complete: `SAVE_FORMAT_VERSION = 24`, single aggregate `survivalEconomyState`, minimum pressure ledger, root BGM audio runtime-promotion review, audio manifest checks in `check:runtime-assets`, and project-level per-small-version branch switching. b2 must not open formal inventory/currency/price table/trade settlement, real consumption/maintenance/refinement-failure settlement, black market/commission/stable arbitrage, DeepSeek economy authority, live DeepSeek drift probe, public wording, BFF/backend, or automatic deployment without explicit user approval. v1.2 must inherit v0.15 low-rank economy/refinement/feeding/caravan/gray-trade boundaries and v1.1 D-025 hidden-name/DeepSeek-drift samples.
