...
- Save format: `SAVE_FORMAT_VERSION = 23`; `v0.11.0-a2` runtime added persistent `livingWorldState`, and `v1.1.0-b1` added the single aggregate `routeLocationState` with migration/defaults/tests in the same change.
...
- `v0.9.0` through `v1.1.0` are completed or locked milestones. Their phase lists, commit/run evidence, and detailed records are indexed in `指导大纲/historical-index.md`; do not treat old version docs as current gates unless AGENTS, PROJECT-STATE, current version docs, or a project-level process explicitly points to them.
...
- `v1.2.0` is the current active draft: `低阶蛊师生存与经济正式化第一阶段`. User approved D-120-001 through D-120-010 and D-121-001 through D-121-007. a1/a2 gates are complete, and b1 projection-only runtime is locally implemented: `src/engine/v120-low-rank-survival-economy-projection.ts`, `LowRankSurvivalEconomyPanel`, and the world-panel `生存` tab display low-rank survival pressure from existing `livingWorldState`, `routeLocationState`, `materialBag`, and v0.15 low-rank economy rules. b1 passed focused unit, `tsc`, focused e2e, 20-round Player Advocate, T0-lite long narrative drift check with live probe exempted, related v0.15/v1.2 unit regression, build, and player-visible-copy scan. Do not add actual `SAVE_FORMAT_VERSION = 24`, `survivalEconomyState`, persistent economy fields, formal inventory/currency/price table/trade settlement, real consumption/maintenance/refinement-failure settlement, black market/commission/stable arbitrage, DeepSeek economy authority, live DeepSeek drift probe, public wording, BFF/backend, or automatic deployment without explicit user approval. If persistent economy state is later approved, use one aggregate pressure-ledger `survivalEconomyState` rather than scattered fields. v1.2 must inherit v0.15 low-rank economy/refinement/feeding/caravan/gray-trade boundaries and v1.1 D-025 hidden-name/DeepSeek-drift samples.
...
- Current active draft docs: `指导大纲/v1.2.0/codex/00-总览/`.
- Current completed milestone docs: `指导大纲/v1.1.0/codex/00-总览/`.
