...
- `v0.17.0` is complete as a local development milestone: `战斗、杀招、小队与阵法深化`. MiroFish v0.17 low-rank combat, killer-move counter, and squad/formation packages passed intake review and were absorbed only as RebornG-owned candidate rules/tests/UI prompts. Runtime adds `src/canon/v017-combat-deepening-rules.json`, `src/engine/v017-combat-deepening.ts`, `V017CombatBoundaryPanel`, ActionPanel/GuDaoPanel/NarrativeCombatPanel bridges, and `registerV017CombatCandidateAction`, all using existing v22 fields and the existing combat-candidate queue. `v0.17.0-rc` passed focused tests, `tsc`, v0.17 e2e, full tests, build, runtime/Qingmao/player-visible-copy scans, full e2e, long e2e, production-preview smoke, b1/b2/b3/b4 20-round Player Advocate gates, rc 60-round Player Advocate gate, and GitHub Actions runs `26046493464` / `26046685585`; implementation commit is `005c1da`, evidence commit is `9c77935`. It adds no save fields, no DeepSeek authority, no formal rewards/drops/materials/Gu/locations/factions/NPC life, no `enemySpecIds`, no backend/BFF, no automatic deployment, and no public commitment. Current next step is stopping for user decision before `v0.18.0` startup review. Stop before save-format changes, DeepSeek authority expansion, formal rewards/locations/factions/NPC life, MiroFish-blocking canon work, external runtime dependencies, backend/BFF, automatic deployment, or public commitments.
- `v0.18.0` is active as `南疆路线与多区域承接`. User approved D-001 through D-008 on 2026-05-19. `v0.18.0-a1` route/location/save/BFF design gate and `v0.18.0-a2` MiroFish intake + field table are complete as documentation/data gates only; no runtime code, no save fields, no DeepSeek authority, no formal route/location/faction/reward/NPC-life result was added. MiroFish packages `v018_route_entry_state_and_milestones_pack`, `v018_southern_border_low_rank_region_fact_cards_pack`, and `v018_post_qingmao_pressure_reaction_pack` were delivered under `指导大纲/vMiroFish/intake-reviews/v0.18.0/` and passed intake review; pressure package item `v018_hidden_982eba1c3730` is quarantined/deferred only. Next step is `v0.18.0-b1` Qingmao route-entry threshold sample, restricted to existing v22 fields and the unified action protocol. Stop again before route/location persistent fields, `SAVE_FORMAT_VERSION = 23`, formal route/location entry, region/faction/reward/NPC-life outcomes, complete Southern Border, full Shang clan city, BFF/backend, or DeepSeek authority expansion.
...
- MiroFish handoff is now part of the phase gate for canon/IF/NPC/faction/route/hidden-fact work. Current v0.18 request drafts are under `指导大纲/vMiroFish/requests/v0.18.0/`; v0.12 and v0.17 packages passed intake review and may feed only candidate/rule drafts. MiroFish output is not canon, not runtime authority, and not DeepSeek authority.
...
For fuller current facts, read `.codex/skills/reborn-expert-council/references/PROJECT-STATE.md` first, then `指导大纲/v0.18.0/codex/00-总览/README.md`, then `指导大纲/v0.17.0/codex/00-总览/README.md`.
...
- Current active draft docs: `指导大纲/v0.18.0/codex/00-总览/`
- Current completed docs: `指导大纲/v0.17.0/codex/00-总览/`
...
- v0.18 startup draft docs: `指导大纲/v0.18.0/codex/00-总览/README.md`, `v0.18.0-总体开发大纲.md`, `v0.18.0-启动审查与范围冻结.md`, `v0.18.0-小版本执行路线图.md`, `v0.18.0-MiroFish资料需求与交付协议.md`, `v0.18.0-测试矩阵.md`, `v0.18.0-真相源索引.md`, `v0.18.0-需求决策池.md`, and `v0.18.0-Git提交与推送计划.md`
