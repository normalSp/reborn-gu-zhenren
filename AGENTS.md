...
- `v1.3.0-rc` is the current active draft: `NPC 与势力长期关系第二层`, local closure complete but live probe failed. User approved D-130-001 through D-130-009, D-131-001 through D-131-007, and later approved D-130-009 scheme 3 (5 samples x 4 rounds, one clean re-probe). b1-b4 are complete locally: world panel adds `社会`, `buildV130SocialPressureProjection()` reads existing `livingWorldState.knownFacts`/`npcMemories`/`factionPressure`/`actionConsequences` and local action ledger, and the UI shows faction pressure, memory traces, public events, social follow-up candidates, projection audit, NPC contact windows, and faction preconditions. v1.3 stays projection-only, does not bump `SAVE_FORMAT_VERSION = 25`, does not add `socialRelationState`, and keeps legacy fields like `npcRelations`/`standings`/`npcContacts`/`dynamicNPCs` out of v1.3 authority unless a later normalization gate promotes them. a2 reuses v0.13 MiroFish NPC memory/motive, faction pressure, and public event chronicle packs as candidate/rule/test material; the full-book base is only archive/source-pointer inventory and coverage reference. Formal named NPC allowlists, wanted/recruitment/blockade conclusions, hidden-adjacent/Fang Yuan public-evidence consequences, NPC life/death, or canon promotion remain blocking and require separate user approval. rc local gates passed: process-1, process-2 deterministic review, 100-round Player Advocate, full unit, typecheck, build, and long e2e. High-strength live probe first run failed at 16/20 accepted, P0/P1/P2 = 7/0/10; after prompt/evaluator and C27 guard strengthening, clean re-probe also failed at 14/20 accepted, P0/P1/P2 = 8/0/8. Do not claim `v1.3 complete`. Further live calls, switching to runtime-guarded acceptance, or lowering/adjusting the live gate require a new user decision. Current docs: `指导大纲/v1.3.0/codex/00-总览/`, especially `v1.3.0-rc-live-probe执行记录.md`, `v1.3.0-rc-live-probe执行方案待确认.md`, `v1.3.0-rc-质量收束记录.md`, and `v1.3.0-rc-Player-Advocate-100轮走查记录.md`, plus project process `指导大纲/流程制度/Skill同步审计制度.md`.
