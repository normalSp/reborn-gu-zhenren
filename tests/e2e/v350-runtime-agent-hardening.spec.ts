...
test.describe('v3.5 transient AgentProposal hardening', () => {
  test('shows lifecycle v2 states and 60-round deterministic gate without persistent agent state', async ({ page }) => {
    ...
    const panel = page.locator('[data-testid="v350-runtime-agent-hardening-panel"]:visible');

    await expect(panel.locator('[data-testid="v340-agent-proposal-status"]')).toContainText('v3.5 lifecycle v2');
    await expect(panel.locator('[data-testid="v350-agent-proposal-lifecycle-audit"]')).toContainText('candidate / rejected / expired / needs_user_decision');
    await expect(panel.locator('[data-testid="v350-agent-proposal-lifecycle-summary"]')).toContainText('候选');
    await expect(panel.locator('[data-testid="v350-agent-proposal-lifecycle-summary"]')).toContainText('阻断');
    await expect(panel.locator('[data-testid="v350-agent-proposal-lifecycle-summary"]')).toContainText('过期');
    await expect(panel.locator('[data-testid="v350-agent-proposal-lifecycle-summary"]')).toContainText('待决策');
    await expect(panel.locator('[data-testid="v350-agent-proposal-lifecycle-details"]')).toContainText('持久化写入候选');
    await expect(panel.locator('[data-testid="v350-agent-proposal-lifecycle-details"]')).toContainText('过期窗口候选');
    await expect(panel.locator('[data-testid="v350-agent-proposal-lifecycle-details"]')).toContainText('future gate 待决策候选');
    await expect(panel.locator('[data-testid="v340-agent-proposal-deterministic"]')).toContainText('60 轮 v3.5');
    await expect(panel.locator('[data-testid="v350-agent-proposal-deterministic-result"]')).toContainText('accepted 60/60');
    await expect(panel.locator('[data-testid="v350-agent-proposal-deterministic-result"]')).toContainText('acceptedForGate true');
