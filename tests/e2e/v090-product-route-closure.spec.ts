...
test.describe('v0.9.0-a1 product route closure', () => {
  test('desktop shows training-ground blockers, debug demo grouping, and a1 version stamp', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    const consoleErrors = await openMidgameDemo(page);

    await expect(page.getByTestId('game-screen-shell')).toBeVisible();
    await expect(page.getByTestId('app-version-label')).toContainText('v0.9.0-a1');
    ...
    await expect(page.locator('[data-testid="training-ground-legacy-policy"]:visible')).toContainText('v0.9.0-a1');
    await expect(page.locator('[data-testid="training-ground-empty-policy"]:visible')).toContainText('当前没有可进入道场');
    await expect(page.locator('[data-testid="training-ground-empty-policy"]:visible')).toContainText(/地点|剧情|线索/);
