...
test.describe('v1.3 social pressure projection', () => {
  test('shows projection-only social pressure without save-format or formal relation writes', async ({ page }) => {
    ...
    await expect(panel.locator('[data-testid="v130-npc-contact-windows"]')).toContainText('NPC 接触窗口');
    await expect(panel.locator('[data-testid="v130-npc-contact-windows"]')).toContainText('no relationship score');
    await expect(panel.locator('[data-testid="v130-npc-contact-windows"]')).toContainText('不写好感度');
    await expect(panel.locator('[data-testid="v130-faction-preconditions"]')).toContainText('势力前置条件');
    await expect(panel.locator('[data-testid="v130-faction-preconditions"]')).toContainText('no formal result');
    await expect(panel.locator('[data-testid="v130-faction-preconditions"]')).toContainText('不写通缉');
