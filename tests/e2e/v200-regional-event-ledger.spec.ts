...
test.describe('v2.0 regional event ledger', () => {
  test('syncs v25 WorldCore regionalEventLedger without formal outcomes or hidden leakage', async ({ page }) => {
    ...
    await expect(panel.locator('[data-testid="v200-replay-diff-audit"]')).toContainText('同开局差异');
    await expect(panel.locator('[data-testid="v200-replay-diff-audit"]')).toContainText('no runFingerprint');
    await expect(panel.locator('[data-testid="v200-ledger-readiness-audit"]')).toContainText('旧档与回滚');
    await expect(panel.locator('[data-testid="v200-ledger-readiness-audit"]')).toContainText('不写新字段');
