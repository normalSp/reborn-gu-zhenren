...
主线候选：`transient AgentProposal` 首刀后的硬化与小样本扩展门禁
...
v3.5 不应该直接扩大成完整 runtime agent。专家团建议先做硬化优先的小版本：验证 v3.4 首刀的边界、玩家理解、重复/空转、同开局差异、rejection 可读性和回滚证据；若用户批准，可在仍然 `transient proposal-only` 的范围内稍微扩展 synthetic L2/L3 样本。
...
请审阅 `v3.5.0-前置授权包.md`。若同意，可批准：

- D-350-001 至 D-350-012。
- F-350-001 至 F-350-012 全部保持 `future_gate_required`。

批准后，v3.5 可以在 `/goal` 内连续完成；触发例外停机清单则必须停止并回到用户决策。
