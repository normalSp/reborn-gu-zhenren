...
日期：2026-05-24
状态：startup opened；等待用户审批 D-300/F-300
主线：`有限 L2/L3 runtime agent 试点设计门禁`

## 一句话结论

v3.0 可以开专家团/设计门禁，但当前仍未批准 runtime agent implementation。

本版本建议先把“一个小区域内有限 L2/L3 agent 输出候选和表达”拆成可批准、可测试、可回滚的工程门禁。只有 D-300/F-300 获批后，才能进入后续 a0/a1/a2 或 runtime 第一刀；在审批前不得改 runtime、save、prompt、DeepSeek、后端、MiroFish 或外部框架依赖。
...
| `v3.0.0-前置授权包.md` | D-300/F-300 一次性审批包 | pending user approval |
| `v3.0.0-需求决策池.md` | 决策池与 future gate 状态 | pending user approval |
...
获批后才能进入 `v3.0.0-a0-v2.9复盘与runtime-agent试点范围冻结.md`。
