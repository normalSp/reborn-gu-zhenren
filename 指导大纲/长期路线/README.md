...
日期：2026-05-23
状态：项目-owned 长期方向；不直接等同当前版本范围；v2.3 已完成，v2.4-a2 已完成，v2.0-v4.0 分层 Agent 与世界内核研究线、v2.0-v3.0 Agent Lab 到 Runtime Agent 总纲、v2.4 前置 Agent Framework Landscape 2026 吸收矩阵已落地并同步。
...
`v2.0.0` 已完成第一个区域活世界本地里程碑，`v2.1.0` 至 `v2.3.0` 已完成 Agent Lab report-only/offline 地基、扩展模拟和 eval farm。`v2.4.0` startup/a0 已开并完成，D-240/D-241 已获用户批准，a1 薄 BFF / private canon / eval archive 边界设计门禁和 a2 external framework adapter / license-SBOM / self-learning 门禁已完成；当前等待用户审批 D-242。继续 v2.4 前，先阅读 `Agent-Framework-Landscape-2026吸收矩阵.md` 和 `指导大纲/v2.4.0/codex/00-总览/README.md`，确认 Hermes/self-learning/外部框架只作为架构吸收与后续评估输入，不直接进入 runtime。

当前建议顺序：

1. 先审阅 `指导大纲/v2.4.0/codex/00-总览/`，确认 v2.4-a1/a2 已完成和 D-242 待批范围。
