...
日期：2026-05-23
状态：项目-owned 长期方向；不直接等同当前版本范围；v2.4 rc 已完成，v2.0-v4.0 分层 Agent 与世界内核研究线、v2.0-v3.0 Agent Lab 到 Runtime Agent 总纲、v2.4 前置 Agent Framework Landscape 2026 吸收矩阵已落地并同步。
...
`v2.0.0` 已完成第一个区域活世界本地里程碑，`v2.1.0` 至 `v2.3.0` 已完成 Agent Lab report-only/offline 地基、扩展模拟和 eval farm。`v2.4.0` 已完成 rc，本版本通过 D-240/D-241/D-242/D-249/D-259/D-269，收束 a1 薄 BFF / private canon / eval archive 边界设计门禁、a2 external framework adapter / license-SBOM / self-learning 门禁、b1 report-only 基础设施边界报告、b2 private canon / hidden leak 风险模型、b3 eval archive / replay / cost / prompt hash schema 草案和 rc v2.5 go/no-go。当前等待用户审批 D-270。继续 v2.5 前，先阅读 `Agent-Framework-Landscape-2026吸收矩阵.md` 和 `指导大纲/v2.4.0/codex/00-总览/README.md`，确认 Hermes/self-learning/外部框架只作为架构吸收与后续评估输入，不直接进入 runtime。

当前建议顺序：

1. 先审阅 `指导大纲/v2.4.0/codex/00-总览/`，确认 v2.4-a1/a2/b1/b2/b3/rc 已完成和 D-270 待批范围。
