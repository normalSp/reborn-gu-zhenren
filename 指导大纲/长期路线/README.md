...
日期：2026-05-26
状态：项目-owned 长期方向；不直接等同当前版本范围；v4.0-v5.0 Auto-Theater 到高阶世界 Runtime 总纲、v3.0-v4.0 Runtime Agent 到高阶世界总纲、v2.0-v4.0 分层 Agent 与世界内核研究线、v2.0-v3.0 Agent Lab 到 Runtime Agent 总纲、v2.4 前置 Agent Framework Landscape 2026 吸收矩阵已落地并同步。
...
`v2.0.0` 已完成第一个区域活世界本地里程碑，`v2.1.0` 至 `v2.3.0` 已完成 Agent Lab report-only/offline 地基、扩展模拟和 eval farm。`v2.4.0` 至 `v2.6.0` 已完成基础设施、private canon、eval archive、job/replay/archive 边界预备。`v2.7.0` 至 `v2.9.0` 已完成多 NPC、小势力、runtime admission 和 v3.0 前安全收束。`v3.0.0` 已完成有限 L2/L3 runtime agent 试点设计门禁，但 `runtimeImplementationApproved=false`，F-300 仍全部为 `future_gate_required`。当前 `v4.0.0` 是 Auto-Theater Combat 与 HeavenWill/Fate Pressure 的启动/设计门禁，不是高阶 runtime 批准。

当前建议顺序：

1. 先审阅 `指导大纲/v4.0.0/codex/00-总览/`，确认 v4.0 是启动/设计门禁，D-400 仍待审批，F-400 仍建议 future gate。
2. 再审阅 `v4.0-v5.0-AutoTheater到高阶世界Runtime总体大纲.md`，确认 v4.1-v4.9 如何把 Auto-Theater / 宏观压力送到 v5.0 第一刀。
3. 再审阅 `v3.0-v4.0-RuntimeAgent到高阶世界总体大纲.md`，确认 v3.1-v3.9 到 v4.0 的证据链来源。
4. 再审阅 `v2.0-v3.0-AgentLab到RuntimeAgent总体大纲.md`，确认 v2.1-v2.9 到 v3.0 的证据链来源。
5. 再审阅 `Agent-Framework-Landscape-2026吸收矩阵.md`，确认外部框架吸收边界。
6. 再审阅 `v2.0-v4.0-分层Agent与世界内核/`，作为更远期的分层 agent、WorldCore、BFF、高阶战斗和宿命/天道研究线。
7. 研究线可以讨论任意身份、五域两天、NPC 思想、天道/宿命和蛊仙战斗，但不能自动变成 runtime 批准。
8. 当前不把外部 AI 人生/跑团项目的“0 岁到多年人生模拟”作为近期可交付目标。RebornG 近期要解决的是 Auto-Theater Combat、Combat Ledger、HeavenWill/Fate pressure、MiroFish/lore 准入和 v5.0 前安全收束，而不是直接开放完整高阶世界。
