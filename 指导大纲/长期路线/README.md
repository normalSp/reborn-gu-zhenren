...
日期：2026-05-18
状态：项目-owned 长期方向；不直接等同当前版本范围；v0.16 UI 减法与系统收束路线已批准并建立前置文档。
...
`v0.10.0` 到 `v0.15.0` 已完成。`v0.16` 已批准为 `系统收束、UI 减法与权威归并`，前置文档已建立于 `指导大纲/v0.16.0/codex/00-总览/`。

当前顺序：

1. 进入 `v0.16.0-a1` 信息架构与权威表设计门禁，不直接改 runtime。
2. 合并 `行动 + 自由目标`，迁出调试/演示入口，归并 `蛊道`。
3. 做旧 NPC/势力系统与 `livingWorldState` 权威表。
4. 做旧 material/shop/refine/economy 与 v0.15 低阶经济权威表。
5. v0.17 再进入战斗、杀招、小队与阵法深化。
6. v0.18 再进入南疆路线与多区域承接。
