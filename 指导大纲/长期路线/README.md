...
`v1.0.0` 已完成为“活世界早期正式版”本地开发里程碑。当前不再沿用 v0.16-v1.0 旧顺序推进，而是进入 v1.1-v2.0 长期路线重整。

当前建议顺序：

1. 先审阅 `RebornG-v1.1至v2.0长期路线重整草案.md`。
2. 再审阅 `指导大纲/v1.1.0/codex/00-总览/` 的启动包。
3. v1.1 第一阶段只做 route/location/save-format 设计门禁，不直接改 runtime。
4. 若用户批准字段方案，再考虑 `SAVE_FORMAT_VERSION = 23` 和正式路线状态第一刀。
5. 当前不把外部 AI 人生/跑团项目的“0 岁到多年人生模拟”作为近期目标。RebornG 近期要解决的是玩家离开青茅山后，路线、地点、区域压力、NPC/势力反应、低阶生存和原著边界能长期一致。
