...
日期：2026-05-18
状态：文档启动已建立，等待进入 `v0.15.0-a1` 设计门禁
主题：`低阶蛊师经济、补给、炼养用深循环`
...
- 进入 `v0.15.0-a1 设计门禁`，先收束字段、权限、测试矩阵和 stop gates，再写 runtime。
- 若 a1 需要把资源、材料或市场规则推入运行时，必须先确认不新增持久字段；若需要新增字段，停下来让用户决策 save format。
