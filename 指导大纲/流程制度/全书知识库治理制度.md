...
从 v1.7-a1 起，任何从全书基础包派生 topic slice 的工作，都必须走 `RebornG request -> MiroFish export -> RebornG intake review -> 测试矩阵/规则草案` 的双仓流水线。MiroFish 仓库只负责 quote-redacted export 和 coverage；RebornG 仓库负责 request、intake、晋升、测试和用户决策。该流水线不得绕过本制度的三层结构和 hidden/private 可见性边界。
...
- `check:mirofish-dual-repo-pipeline`：v1.7 起检查双仓路径、基础包边界、MiroFish export 工具入口、request/export/intake/test 矩阵证据链。
