...
## v0.8.0-c2.6-c2.8 世界闭环动效分工

c2.6-c2.8 不新增大型视觉系统，而是修正已有界面的信息层级和动效边界：

Motion/CSS 负责：
- 群像战阶段条、已行动/剩余单位、结束我方阶段按钮和轨迹列表状态。
- 终局详情的展开/收起、证据列表、候选结局和压力日志。
- 空窍/仙窍内调息、修行、突破、升仙、灾劫入口的卡片和轨迹列表。
- 背包用蛊意图提示、目标选择弹层、传承线索账本、出发/试炼/认主按钮状态。

GSAP 负责：
- `battlefield-gsap-*` 独立 effect layer 中的战斗强调，但层级必须高于棋盘和单位，且保持 `pointer-events: none`。
- 后续传承封印开启、地灵现形、认主成败等独立效果层；不得控制传承线索列表、按钮、文字或布局。

边界：
- 背包用蛊不播放“已生效”强演出，只提示“已登记剧情用蛊意图”，避免和剧情选择/场景 AP 冲突。
- 终局候选默认折叠，不用动画暗示玩家可手动挑选具体结局；正式结果仍由本地 `commitBestEndingCandidateAction()` 自动选择。
- 传承面板不再展示样板入口按钮；动效只表现剧情线索账本和本地结算轨迹，不暗示可以无剧情线索刷奖励。
- reduced motion 下保留阶段、AP、线索、地灵条件、出发结果和本地轨迹，不依赖闪烁或大幅位移表达状态。
