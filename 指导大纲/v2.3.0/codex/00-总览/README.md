...
状态：v2.3.0 startup draft；等待用户审批 D-230。
日期：2026-05-23
当前分支：`codex/v230-startup-agent-eval-farm`

## 当前一句话

`v2.3.0` 建议主线为 `Agent eval farm 与失败分类硬化`。
...
- v2.3 startup / a0 文档草案已建立。
- D-230-001 至 D-230-012：等待用户审批。
- 下一步若 D-230 批准：进入 `v2.3.0-a1-failure-taxonomy与severity-rubric设计门禁.md`。

## v2.3 推荐切法

1. startup/a0：v2.2 runner 复盘、v2.3 eval farm 范围冻结、D-230 决策池。当前草案。
2. a1：failure taxonomy 与 severity rubric 设计门禁。等待 D-230。
3. a2：eval farm schema、report archive、trend ledger 设计门禁。等待 a1 后决策。
4. b1：若获批，实现自有 zero-dependency report-only eval farm runner。默认不写 runtime。
5. b2：负例注入、deterministic mutation、replay/rescore 和 false-negative gate。
6. b3：报告归档、趋势比较、future_sample_pool 回流。
...
请用户审批 D-230。D-230 批准后，进入 a1 设计门禁，不直接写 runner。
