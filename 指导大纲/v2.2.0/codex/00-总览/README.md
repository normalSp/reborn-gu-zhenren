...
状态：v2.2.0-a2 completed；等待 D-222 决策。
日期：2026-05-23
当前分支：`codex/v220-a2-memory-visibility-postcheck`

## 当前一句话

`v2.2.0` 主线已按用户批准的 D-220 冻结为 `Agent Lab 扩展离线模拟第一阶段`。a1 已完成 20 NPC / 3 势力 / 1 个 L5 宏观导演的离线场景模型设计门禁；a2 已完成 memory / visibility / WorldCore post-check 扩展门禁。下一步等待 D-222 决定是否进入 b1 自有 offline runner 扩展第一刀。
...
4. b1：若用户批准 D-222，扩展自有 report-only/offline runner 到 v2.2 样本规模。
...
见 `v2.2.0-需求决策池.md` 的 D-222-001 至 D-222-010。

在 D-222 批准前，不进入 b1，不写 runner 扩展，不新增样本文件或 artifact。
