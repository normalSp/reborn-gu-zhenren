...
状态：v2.4 专家团启动会已开；D-240 决策池待用户审批；不授权 runtime。
日期：2026-05-23
当前分支：`codex/v240-long-route-sync-startup`
...
- 下一步：由用户审批 D-240-001 至 D-240-012 后，进入 v2.4-a1 设计门禁。

## 推荐切法

1. startup/a0：长期路线修正、Landscape 同步、v2.4 范围冻结。已完成。
2. a1：薄 BFF / private canon / eval archive 边界设计门禁。
3. a2：外部 framework adapter / license-SBOM / self-learning candidate pipeline 门禁。
...
专家团建议先让你审批 D-240。审批后再进入 a1，不要绕过设计门禁直接实现任何 backend、PoC 或 runtime agent。
