...
type ScreenState = 'input_key' | 'testing' | 'result';
...
function App() {
  ...
  const handleTestConnection = async () => {
    ...
    if (response.success && response.data) {
      ...
      setResultMessage(
        response.data.message || '天道已响应 — API 连通成功'
      );
      setTokenInfo(
        `耗时 ${response.elapsedMs}ms · ` +
        `Tokens ${response.tokens?.total_tokens ?? '?'} ` +
        `(缓存命中 ${response.tokens?.cached_tokens ?? 0})`
      );
    } else {
  ...
  return (
    <div className="min-h-[100dvh] bg-rg-ink-800 flex flex-col items-center justify-center p-8">
      ...
      <div className="text-center mb-12">
        <h1 className="text-5xl font-bold text-rg-gold mb-3 font-narrative tracking-wider">
          蛊真人世界
        </h1>
        <p className="text-rg-paper-100 text-lg font-panel tracking-[0.15em]">
          人生重来模拟器
        </p>
        <div className="mt-6 w-16 h-[1px] bg-rg-gold/40 mx-auto" />
      </div>

      {/* ─── 主面板 (surface-raised) ─── */}
      <div className="w-full max-w-md bg-rg-ink-700 border border-rg-ink-400/25 rounded-lg p-8 shadow-lg shadow-black/40">
        {/* API Key 输入区 */}
        <label className="block text-rg-paper-100 text-sm font-panel mb-2">
          蛊界天道密钥
        </label>
        ...
        {screenState === 'result' && (
          <div className={`mt-6 p-4 rounded-sm border-2 ${
            resultSuccess
        ...
        <div className="mt-8 pt-4 border-t border-rg-ink-400/20">
          <p className="text-rg-ink-300 text-xs font-panel text-center">
            蛊真人世界 · 人生重来模拟器 · v0.1.0
          </p>
          <p className="text-rg-ink-400 text-xs font-panel text-center mt-1">
            DeepSeek V4 Pro · React 18 · TypeScript 5 · 账簿式 UI
          </p>
        </div>
      </div>
