...
- `v0.13.0` mainline, first cut, v22 field strategy, `v0.13.0-process-0 Git安全收束`, and first two MiroFish requests are approved. Mainline is `NPC 与势力反应系统`; current step is `v0.13.0-process-0`, then `v0.13.0-a1 社会记忆协议、字段表、测试矩阵`. Runtime named NPC/faction rules remain blocked until the relevant MiroFish package is delivered and passes intake review.
...
- v0.13 MiroFish request drafts exist: `qingmao_npc_memory_motive_pack`, `qingmao_faction_reputation_pressure_pack`, and `qingmao_public_event_chronicle_pack`. The user will transfer the first two to MiroFish thread `019e207b-c55d-7e23-b450-efa7a054a165`; the third can wait until b2 public event chronicle.
